...
def test_model(model, env, ntests, name, stats_path=None):
    ...
    for i in range(ntests):
        ...
        for (env_, info_) in zip(env.envs, info):
            ...
            if info_['s_radius'] < 1.0:
                nsucc += 1
                mean_niter += info_['s_radius']
                # Store each iteration count together with the respective
                # lambda to make nice plots later on
                results.append((info_['lam'].real, info_['s_radius']))
                Qd1.append((info_['lam'].real, info_['diag'][1]))
...
def plot_results(results, color, label, mark='-'):
...
def run_tests(model, args, seed=None, fig_path=None, stats_path=None, heat=True):
    ...
    plt.xlabel('re(λ)')
    plt.ylabel('Qd entry / Spectral Radius SDC')

    # Test the trained model.
    env = utils.make_env(args, num_envs=num_test_envs, seed=seed  ,      lambda_real_interpolation_interval=None)
    results_RL, Qd1_RL = test_model( model, env, ntests, 'RL', stats_path=stats_path)
    plot_results(results_RL, color='b', label='RL Spect')
    plot_results(Qd1_RL, color='b', label='RL Qd1', mark='--')
    ...
    if (args.show_MIN):
        ...
        results_min, Qd1_MIN = test_model(model, env, ntests, 'MIN')
        plot_results(results_min, color='g', label='MIN Spec')
        plot_results(Qd1_MIN, color='g', label='MIN Qd1', mark='--')
