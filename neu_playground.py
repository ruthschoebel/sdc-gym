#python -W ignore neu_playground.py --tests 1  --steps 0 --model_path dp_model_2021-05-13T16-27-27.359957.npy
...
def parse_args():
    ...
    parser.add_argument(
        ...
        type=int,
        default=0,
        help='0 test equation, 1 heat equation',
    )
...
def test_model(model, params, env, ntests, name,
    ...
    for (env_, info_) in zip(env.envs, info):
        ...
        if info_['niter'] < 50 and info_['residual'] < env_.restol:
            ...
            #print("lambda", info_['lam'])
            #print("-niter", info_['niter'])
            results.append((info_['nu'].real, info_['niter']))
    ...
    return (info_['nu'].real, info_['niter']) #results
...
def run_tests(model, params, args,
    ...
    num_test_envs = 1#args.batch_size
    ...
    results_min_voll=[]
    example=1
    for nu in [0.001, 0.01, 0.1, 1, 10]:
        nvars = 256
        start_time = time.perf_counter()
        # Test the trained model.
        env = utils.make_env(args, num_envs=num_test_envs, seed=seed,
                             lambda_real_interpolation_interval=None,
                             do_scale=False, example=example, nvars=nvars, dtype=np.complex128, model=model, params=params, nu=nu)


        results_RL.append(  test_model(
            model, params, env, ntests, 'RL', loss_func, stats_path=stats_path))


        env = utils.make_env(args, num_envs=num_test_envs, seed=seed,
                             lambda_real_interpolation_interval=None,
                             do_scale=False, example=example, nvars=nvars, dtype=np.complex128, model=model, params=params, nu=nu, imex=False)


        results_RL_voll.append(  test_model(
            model, params, env, ntests, 'RL', loss_func, stats_path=stats_path))
        ...
        env = utils.make_env(
            ...
            lambda_real_interpolation_interval=None,
            do_scale=False,example=example, nvars=nvars, dtype=np.complex128, model=model, params=params,nu=nu)
        results_LU.append( test_model(model, params, env, ntests, 'LU'))
        ...
        env = utils.make_env(
            ...
            seed=seed,
            lambda_real_interpolation_interval=None,
            do_scale=False,example=example, nvars=nvars, dtype=np.complex128, model=model, params=params,nu=nu, imex=True)
        results_LU_voll.append( test_model(model, params, env, ntests, 'LU'))
        ...
        env = utils.make_env(
            ...
            lambda_real_interpolation_interval=None,
            do_scale=False, example=example, nvars=nvars, dtype=np.complex128, model=model, params=params,nu=nu)
        results_min.append(test_model(model, params, env, ntests, 'MIN'))
        ...
        env = utils.make_env(
            ...
            lambda_real_interpolation_interval=None,
            do_scale=False, example=example, nvars=nvars, dtype=np.complex128, model=model, params=params,nu=nu, imex=True)
        results_min_voll.append(test_model(model, params, env, ntests, 'MIN'))
    #print(results_min)
    ...
    plot_results(results_RL, color='b', label='RL+0',marker=True)
    plot_results(results_LU, color='r', label='LU+EE',marker=True)
    #plot_results(results_min, color='g', label='MIN+0',marker=True)

    plot_results(results_RL_voll, color='y', label='RL+RL')
    plot_results(results_LU_voll, color='c', label='LU+LU')
    plot_results(results_min_voll, color='m', label='MIN+MIN')
    plt.legend()
