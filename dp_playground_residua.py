...
class DataGenerator:
    ...
    def _compute_initial_residual(self, u, C):
        return u - C @ u
    ...
    def __iter__(self):
        while True:
            ...
            u = jnp.ones(self.M, dtype=jnp.complex64)
            residuals = jax.vmap(
                self._compute_initial_residual, (None, 0))(u, Cs)
            yield (residuals, lams)
...
def main():
    ...
    @jax.jit
    def loss(params, residuals, lams):
        diags = model(params, residuals)
        return loss_func(lams, diags)

    @jax.jit
    def update(i, opt_state, residuals, lams):
        params = opt_get_params(opt_state)
        loss_, gradient = jax.value_and_grad(loss)(params, residuals, lams)
        # print(gradient)
    ...
    start_time = time.perf_counter()
    for (step, (residuals, lams)) in enumerate(dataloader):
        loss_, opt_state = update(step + old_steps, opt_state, residuals, lams)
